...
export const handleIssueClosed = async () => {
  ...
  if (issue.body && isParentIssue(issue.body)) {
    logger.error("Permit generation disabled because the issue is identified as parent issue!");
    await clearAllPriceLabelsOnIssue();
    return "Permit generation disabled because the issue is identified as parent issue!";
  }
  ...
  for (const botComment of comments.filter((cmt) => cmt.user.type === UserType.Bot).reverse()) {
    ...
    if (botCommentBody.includes(GLOBAL_STRINGS.autopayComment)) {
      ...
      if (res) {
        if (res[1] === "false") {
          logger.info(`disabled to generate permit2 url, reason: autoPayMode for this issue: false`);
  ...
  if (paymentPermitMaxPrice == 0 || !paymentPermitMaxPrice) {
    logger.info(`disabled to generate permit2 url, reason: { paymentPermitMaxPrice: ${paymentPermitMaxPrice}}`);
    return `Permit generation disabled because paymentPermitMaxPrice is 0`;
  }

  const issueDetailed = bountyInfo(issue);
  if (!issueDetailed.isBounty) {
    logger.info(`disabled... its not a bounty`);
  ...
  if (!assignee) {
    logger.info("Disabled to proceed the payment because `assignee` is undefined");
    return `Permit generation disabled because assignee is undefined!`;
  }

  if (!issueDetailed.priceLabel) {
    logger.info("Disabled to proceed the payment because price not set!");
  ...
  if (priceInEth.gt(paymentPermitMaxPrice)) {
    logger.info("disabled to proceed the payment because bounty payout is higher than paymentPermitMaxPrice");
