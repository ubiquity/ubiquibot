...
export async function issueReopenedCallback() {
  ...
  if (!permitComment) {
    throw logger.error(`Permit comment not found`);
  }
  ...
  if (parseFloat(formattedAmount) > 0) {
    ...
    return logger.info(`Penalty of ${formattedAmount} ${tokenSymbol} deducted from @${assignee}`);
  ...
  throw logger.error(`Issue reopened callback not implemented`);
}
